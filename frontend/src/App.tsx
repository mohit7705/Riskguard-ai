...
function App() {
  ...
  const [entryStep, setEntryStep] = useState<'email' | 'assignment'>('email')
  const [assignmentMode, setAssignmentMode] = useState<'lookup' | 'create'>('lookup')
  ...
  const handleEmailContinue = async () => {
    ...
    if (!value) {
      setEntryError('Please enter your email or assignment number.')
      return
    }
    ...
    if (!isEmail && !isNumber) {
      setEntryError('Enter a valid Gmail/email or assignment number.')
      return
    }
    ...
    if (isNumber) {
      ...
      try {
        ...
        if (existing) {
          ...
          setAssignmentNumber(existing.assignment_number)
        } else {
  ...
  const handleAssignmentLookup = async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error ? err.message : 'Unable to find assignment.'
  ...
  const handleAssignmentCreate = async () => {
    ...
    try {
      ...
      setAssignmentNumber(created.assignment_number)
    } catch (err) {
      setEntryError(
        err instanceof Error
          ? err.message
    ...
    return (
      <div className="entry-shell">
        <div className="entry-card">
          ...
          {entryStep === 'email' && (
            <>
              <div className="entry-heading">
                ...
                <p>
                  Enter your email to continue to your risk
                  intelligence workspace.
                </p>
              </div>

              <div className="entry-form">
                <label htmlFor="email">Email or assignment number</label>
                <input
                  id="email"
                  type="email"
                  value={emailInput}
          ...
          {entryStep === 'assignment' && !assignment && (
            <>
              <div className="entry-heading">
              ...
              <div className="entry-form">
                ...
                {assignmentMode === 'lookup' ? (
                  ...
                  >
                    {entryLoading ? 'Checking...' : 'Open Assignment'}
                  </button>
                ) : (
                  <button
                    type="button"
                    ...
                    disabled={entryLoading}
                  >
                    {entryLoading ? 'Creating...' : 'Create Assignment'}
                  </button>
                )}
